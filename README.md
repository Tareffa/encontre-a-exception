...
Um cliente relatou falhas no envio de guias de pagamento via WhatsApp. Após investigação, descobriu-se que a classe `LegacyCommunicationService` estava lançando uma **Exception não-mapeada** que não era capturada adequadamente, fazendo com que tokens válidos não fossem processados.
